...
# MAGIC       <p style="color: #B0BEC5; margin: 5px 0 0 0;">PySpark DataFrame API で CSV を Bronze テーブル（Delta）に取り込む</p>
# MAGIC     </div>
...
# MAGIC VolumeにあるCSVファイルを PySpark DataFrame API を使って Delta形式の Bronze テーブルとして取り込みます。<br>
# MAGIC 監査カラムの付与や、テーブルへの書き込み方法を体験します。
...
# MAGIC     <td style="padding: 6px 10px;">spark.read + saveAsTable</td>
# MAGIC   </tr>
# MAGIC   <tr>
...
# MAGIC <strong>🔗 PySpark DataFrame API での取り込み</strong><br>
# MAGIC <code>spark.read.format("csv")</code> でCSVファイルを読み込み、<br>
# MAGIC <code>df.write.mode("overwrite").saveAsTable()</code> で Delta テーブルとして保存します。<br>
# MAGIC Python の柔軟性を活かして、読み込みと同時にカラム追加や型変換が可能です。
# MAGIC </div>
...
# MAGIC %md
# MAGIC ---
# MAGIC ## 1. マスタ系テーブルの取り込み

# COMMAND ----------

# DBTITLE 1,CSVの中身をクイック確認
...
df.write.mode("overwrite").saveAsTable(f"bz_stores")
...
df.write.mode("overwrite").saveAsTable(f"bz_products")
...
df.write.mode("overwrite").saveAsTable(f"bz_customers")
...
df = spark.read.format("csv") \
    .option("header", "true") \
    .load(f"{VOLUME_PATH}/weather.csv")

df.write.mode("overwrite").saveAsTable(f"bz_weather")
...
df = spark.read.format("csv") \
    .option("header", "true") \
    .load(f"{VOLUME_PATH}/facilities.csv")

df.write.mode("overwrite").saveAsTable(f"bz_facilities")
...
# MAGIC データ品質チェックのコードを書く必要がありません。
# MAGIC </div>
...
df.write.mode("overwrite").saveAsTable(f"bz_sales")
...
df = spark.read.format("csv") \
    .option("header", "true") \
    .load(f"{VOLUME_PATH}/sale_items.csv")

df.write.mode("overwrite").saveAsTable(f"bz_sale_items")
...
# MAGIC %md-sandbox
# MAGIC <div style="border-left: 4px solid #1976D2; background: #E3F2FD; padding: 12px 16px; border-radius: 4px; margin: 10px 0;">
# MAGIC <strong>🔗 監査カラム（Audit Columns）とは</strong><br>
# MAGIC データ取り込み時にメタ情報として付与するカラムのことです。<br>
# MAGIC <ul>
# MAGIC   <li><code>_ingested_at</code>: いつ取り込んだか（タイムスタンプ）</li>
# MAGIC   <li><code>_source_file</code>: どのファイルから取り込んだか（パス）</li>
# MAGIC </ul>
# MAGIC 障害調査時に「このレコードはいつ・どこから来たか？」を追跡できます。<br>
# MAGIC 本番ETLでは付与するのがベストプラクティスです。
# MAGIC </div>

# COMMAND ----------

# DBTITLE 1,監査カラム付きで取り込む例
from pyspark.sql.functions import current_timestamp, lit

source_path = f"{VOLUME_PATH}/stores.csv"
...
df_with_audit.write.mode("overwrite").saveAsTable(f"bz_stores")

display(spark.table(f"bz_stores").limit(3))
...
# MAGIC <ul>
# MAGIC   <li><a href="https://docs.databricks.com/ja/pyspark/basics.html" target="_blank">PySpark DataFrame 入門</a></li>
...
tables = spark.sql(f"SHOW TABLES LIKE 'bz_*'")
display(tables)
...
# MAGIC 全CSVファイルを PySpark DataFrame API で Delta形式の Bronze テーブルとして取り込みました。<br>

...
# MAGIC     <div>
# MAGIC       <h1 style="color: #FFFFFF; margin: 0; font-size: 26px;">05 | テーブル設定（SQL+Python）</h1>
...
# MAGIC DDLはSQL構文が最も簡潔なため、Python辞書で定義 → <code>spark.sql()</code> ループで一括実行します。
# MAGIC </div>
...
column_comments = {
    "sl_stores": {
        "store_id":       "店舗ID（一意）",
        "store_name":     "店舗名",
        "prefecture":     "都道府県（NULLは「不明」に変換済み）",
        "lat":            "緯度",
        "lon":            "経度",
        "floor_area_sqm": "売場面積（平方メートル）",
        "store_type":     "店舗タイプ（大型/標準/コンパクト）",
    },
    "sl_sales": {
        "sale_id":        "売上ID（一意）",
        "customer_id":    "会員ID（sl_customersへのFK）",
        "store_id":       "店舗ID（sl_storesへのFK）",
        "total_amount":   "合計金額（税込）",
        "payment_method": "決済手段（現金/クレジットカード/電子マネー/QRコード決済/ポイント）",
    },
    "gd_rfm": {
        "r_score":     "Recencyスコア（5=最近購入, 1=長期未購入）",
        "f_score":     "Frequencyスコア（5=高頻度, 1=低頻度）",
        "m_score":     "Monetaryスコア（5=高額, 1=低額）",
        "rfm_segment": "RFMセグメント（ロイヤル顧客/優良顧客/新規顧客/離反リスク/休眠顧客/一般顧客）",
    },
}
...
for table, columns in column_comments.items():
    ...
    for column, comment in columns.items():
        ...
        spark.sql(f"ALTER TABLE {table_path} ALTER COLUMN {column} COMMENT '{escaped_comment}'")
        count += 1

print(f"✅ {count} カラムにコメントを設定しました")
